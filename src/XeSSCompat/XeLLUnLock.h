...
class XeLLUnlock
{
  public:
    ...
    static bool Apply(HMODULE module)
    {
        ...
        {
            logger::warn("XeLL unlock: provider has no usable PE headers, skipping");
            return false;
        }
        ...
        if (build == nullptr)
        {
            logger::warn("XeLL unlock: unrecognised provider build {:#010x}/{:#x}, skipping",
        ...
        logger::debug("XeLL unlock: recognised provider build {:#010x}, gate at {:#x}, {} -> {}", build->stamp,
        ...
        for (const auto& patch : patches)
        {
            ...
            {
                logger::warn("XeLL unlock: {} at {:#x} falls outside .text, aborting", patch.name, patch.rva);
            ...
            {
                logger::warn("XeLL unlock: {} at {:#x} has unexpected bytes ({}), aborting", patch.name, patch.rva,
            ...
            {
                logger::warn("XeLL unlock: {} at {:#x} failed write verification, aborting", patch.name, patch.rva);
                Rollback(applied, appliedCount);
                return false;
            }
            ...
            logger::debug("XeLL unlock: {} patched at {:#x} -> {}", patch.name, patch.rva, ToHex(dst, patch.size));
        }
        ...
        logger::debug("XeLL unlock: {} of {} patch(es) applied, generated frame count allowed up to {}X", appliedCount,
  ...
  private:
    ...
    static constexpr int32_t MaxAllowedInterpolations = 7;
    ...
    static void Rollback(const Edited* applied, int32_t count)
    {
        ...
        logger::debug("XeLL unlock: rolled back {} patch(es)", count);
    }
