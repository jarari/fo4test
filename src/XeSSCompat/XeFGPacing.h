...
// Derived from Coldwood1026/OptiScaler, commit 9eea95bba9fda7121f214d2eba358423be598d7e.
// Modified for Upscaling: 2026-09-19; fixed 8x ceiling, host logging/config adaptation.
// See NOTICE.md in this directory. Intel binaries are not modified on disk.
...
inline void ReportStats(int64_t multiplier)
{
    ...
    logger::debug("XeFG pacing: {}X, real frame {:.2f} ms ({:.1f} fps), target {:.2f} ms/frame; "
             ...
             g_tsRebased, g_tsClamped, g_renderTimeNs.load(std::memory_order_relaxed) / 1000000.0, g_fedFrameTimeMs.load(std::memory_order_relaxed));
}
...
inline void PaceFrame(uint64_t index, uint64_t count)
{
    ...
    if (index <= 1)
    {
        ...
        if (!g_loggedFirstBurst)
        {
            ...
            logger::debug("XeFG pacing: {}X, real frame {:.3f} ms -> {:.3f} ms per generated frame", multiplier,
...
inline void ScheduleFrame(void* ctx, uint8_t* burst, uint64_t index)
{
    ...
    if (g_schedLogged < SchedLogLimit && index != g_schedLastIndex)
    {
        ...
        logger::debug("XeFG pacing: frame {}/{} -> {} in {:.3f} ms "
                      ...
                      void** nativeOut)
{
    ...
    {
        logger::warn("XeFG pacing: thunk at {:#x} has unexpected bytes, not hooking", rva);
        return false;
    }
    ...
    {
        logger::warn("XeFG pacing: failed to write the thunk at {:#x}", rva);
        return false;
    }
...
inline bool Install(uint8_t* base)
{
    ...
    {
        logger::warn("XeFG pacing: no performance counter, not hooking");
        return false;
    }
    ...
    logger::debug("XeFG pacing: generated frames are paced above 2X (thunk {:#x} -> {:#x})", PresentThunkRva,
    ...
    if (HookThunk(base, SchedThunkRva, SchedFnRva, SchedThunkExpected, &SchedForwarder, &native))
        logger::debug("XeFG pacing: scheduling every generated frame through the provider's own scheduler "
                 "(thunk {:#x})",
                 SchedThunkRva);
    else
    {
        g_schedNative = nullptr;
        logger::warn("XeFG pacing: no scheduler, falling back to the wall clock");
    }
    ...
    if (g_schedNative != nullptr &&
        ...
        logger::debug("XeFG pacing: deadlines come from the burst's own frame interval (thunk {:#x})", TimestampThunkRva);
    else
        g_tsNative = nullptr;
