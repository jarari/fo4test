...
class XeFGUnlock
{
  public:
    ...
    static bool Apply(HMODULE module)
    {
        ...
        {
            logger::warn("XeFG unlock: provider has no usable PE headers, skipping");
            return false;
        }

        if (nt->FileHeader.TimeDateStamp != KnownBuildStamp || nt->OptionalHeader.SizeOfImage != KnownSizeOfImage) {
            logger::warn("XeFG unlock: unknown provider layout; retaining stock capabilities");
            return false;
        } else
            logger::debug("XeFG unlock: recognised provider build {:#010x}", KnownBuildStamp);
        ...
        for (const auto& patch : patches)
        {
            ...
            {
                logger::warn("XeFG unlock: {} at {:#x} falls outside .text, aborting", patch.name, patch.rva);
            ...
            {
                logger::warn("XeFG unlock: {} at {:#x} has unexpected bytes ({}), aborting", patch.name, patch.rva,
            ...
            {
                logger::warn("XeFG unlock: {} at {:#x} failed write verification, aborting", patch.name, patch.rva);
                WriteRaw(dst, patch.expected, patch.size);
                Rollback(applied, appliedCount);
                return false;
            }
            ...
            logger::debug("XeFG unlock: {} patched at {:#x} -> {}", patch.name, patch.rva, ToHex(dst, patch.size));
        }

        _applied = true;

        logger::debug("XeFG unlock: {} of {} patches applied ({} skipped), MFG enabled up to {}X", appliedCount, PatchCount,
                 skipped, maxInterp + 1);
  ...
  private:
    ...
    static constexpr int32_t MaxReportedInterpolations = 7;
    ...
    static void Rollback(const Edited* applied, int32_t count)
    {
        ...
        logger::debug("XeFG unlock: rolled back {} patch(es)", count);
    }
