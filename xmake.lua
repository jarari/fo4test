...
target("Upscaling")
    ...
    add_includedirs("extern/XeSS/inc")
    ...
    after_build(function (target)
        ...
        local xess = path.join(os.projectdir(), "extern", "XeSS")
        for _, dll in ipairs({"libxess.dll", "libxess_fg.dll", "libxell.dll"}) do
            os.cp(path.join(xess, "bin", dll), target:targetdir())
        end
        os.cp(path.join(xess, "LICENSE.txt"), path.join(target:targetdir(), "XeSS-LICENSE.txt"))
    end)
